import { registerUser, ApiItemResponse, User } from "../api/api";

const form = document.querySelector<HTMLFormElement>("#signupForm");
const nicknameInput =
  document.querySelector<HTMLInputElement>("#nicknameInput");
...
const submitButton =
  document.querySelector<HTMLButtonElement>(".signup__submit");
...
const fieldElements = {
  ...
  passwordConfirm: document.querySelector<HTMLDivElement>(
...
function setFieldState(
  ...
  const messageElement =
    fieldElement.querySelector<HTMLParagraphElement>(".field__message");
  if (messageElement) {
...
async function handleSubmit(event: SubmitEvent): Promise<void> {
  ...
  if (!response.ok) {
    ...
    if (response.message?.includes("이메일")) {
      ...
      response.message?.includes("별명") ||
      response.message?.includes("닉네임")
    ) {
      duplicateState.nicknameChecked = false;
